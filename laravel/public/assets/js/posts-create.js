...
(function() {
  ...
  const yearBox = document.getElementById('to_year');
  const monthBox = document.getElementById('to_month');
  const dayBox = document.getElementById('to_day');
  ...
  monthBox.addEventListener('change', (e) => {
        dayBox.innerHTML = '';
        const selectedMonth = e.target.value;
        createOption('to_day', 1, daysOfYear[selectedMonth - 1], 1);
    });
    ...
    yearBox.addEventListener('change', e => {
        ...
        createOption('to_month', 1, 12, 1);
        createOption('to_day', 1, daysOfYear[0], 1);
    });

    createOption('to_year', thisYear, thisYear + 1, thisYear);
    createOption('to_month', 1, 12, thisMonth);
    createOption('to_day', 1, daysOfYear[thisMonth - 1], thisDay);
})();
